...
function put_cache() {
  ...
  client1.select('2', function(error){
    client1.get('WEB:LINK:link', function(error, res){
      if(error){
        console.log(error);
      }else{
        //更新node cache 缓存
        //console.log(JSON.parse(res));
        node_cache.put('pageLink', JSON.parse(res));
      }
    });
  });
}
...
function getRedisData() {
  ...
  client.on("ready", function () {
    //订阅消息
    client.subscribe("WEB:LINK:link_message");
  ...
  client.on("message", function (channel, message) {
    ...
    //dealWithMsg(message);
    //更新node cache 缓存
    node_cache.put('pageLink', JSON.parse(message));
  });
